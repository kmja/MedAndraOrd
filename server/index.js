...
app.post('/api/clue', async (req, res) => {
  const pid = getPlayerId(req, res);
  if (rateLimited(pid)) {
    return res.status(429).json({ error: 'För många försök på kort tid. Vänta en stund.' });
  }
  ...
  if (player.attempts >= MAX_ATTEMPTS) {
    return res.status(403).json({ error: 'Du har inga försök kvar idag. Välkommen tillbaka imorgon!' });
  }
  ...
  if (!verdict) {
    ...
    try {
      ...
      if (err instanceof AiUnavailableError) {
        return res.status(503).json({ error: 'AI:n går inte att nå just nu. Försök igen om en stund — inget försök förbrukades.' });
      }
